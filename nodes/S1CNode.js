    ...
    function broadlinkNode(config) {
        ...
        this.on('input', function (msg) {
            ...
            if (conf != null && conf != undefined && conf != "") {
                ...
                var _device = new S1C({ address: msg.payload.host, port: 80 }, Buffer.from(msg.payload.mac.replace(':', '').replace('-', '').match(/[0-9A-Fa-f]{2}/g).map(function (num) { return parseInt(num, 16); })));
            }
